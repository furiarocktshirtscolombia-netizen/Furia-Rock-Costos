...
export default function TopBar(props: Props) {
  const {
    ...
    onExportPNG, onExportPDF, onPreview, onAddToCart, onSendToProduction, onOpenHelp, autosaveLabel,
  } = props;
  ...
  return (
    <div className="w-full flex flex-wrap items-center justify-between gap-3 bg-[#121317] border border-white/10 rounded-2xl p-3 mb-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={onOpenHelp} className="btn-light text-xs" title="Guia de uso del disenador">? Ayuda</button>
      </div>
    </div>
  );
}
