...
const SEDES         = ["MedellÃ­n","BogotÃ¡","Cali","Online","Otra"];
...
function calcPrice(ref:Ref, qty:number, cmEstampado:number, empaque:number, costoImpExtra:number): Calc {
  const ganancia = ref.cat==="NiÃ±o" ? 9000 : 14000;
...
// âââ COMPONENTES REUTILIZABLES âââââââââââââââââââââââââââââââââââââââââââââââ
...
const Alert = ({msg,type,onClose}:{msg:string;type:"green"|"red"|"yellow";onClose:()=>void}) => {
  ...
  return (
    <div style={{background:c.bg,color:c.fg,border:`1px solid ${c.bd}`,borderRadius:8,padding:"10px 14px",fontSize:13,marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
      ...
      <button onClick={onClose} style={{background:"transparent",color:c.fg,padding:"2px 8px",fontSize:12}}>â</button>
    </div>
  );
};
...
export default function App() {
  ...
  const [sede,      setSede]      = useState("MedellÃ­n");
  ...
  const tallas   = ref.cat==="NiÃ±o"?TALLAS_NINO:TALLAS_ADULTO;
  ...
  const addToQuote = () => {
    ...
    if(st>=0&&qty>st){ showAlert(`â ï¸ Stock insuficiente: ${st} unidades disponibles de ${ref.name} (${talla}/${color})`,"yellow"); return; }
    setQuote(q=>[...q,{id:uid(),ref,refId,talla,color,qty,calc,diseno,tipoImp,sede,cliente}]);
    showAlert(`â "${ref.name}" agregado a la cotizaciÃ³n.`);
  };

  const registrarVenta = () => {
    if(!quote.length){ showAlert("Sin items en la cotizaciÃ³n.","red"); return; }
    for(const item of quote){
      const st=stockDe(item.refId,item.talla,item.color);
      if(st>=0&&item.qty>st){ showAlert(`â Sin stock: ${item.ref.name} (${item.talla}/${item.color}) â disponible: ${st}`,"red"); return; }
    }
    const fecha=today();
    ...
    <>
      ...
      <nav style={{background:"#16181f",borderBottom:"1px solid #2a2d3a",display:"flex",alignItems:"center",padding:"0 1rem",gap:0,position:"sticky",top:0,zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px 12px 0",borderRight:"1px solid #2a2d3a",marginRight:8}}>
          <div style={{width:32,height:32,background:"#e63d3d",borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>â¡</div>
          <div>
            <div style={{fontWeight:700,fontSize:15,letterSpacing:.5,color:"#fff"}}>FURIA ROCK</div>
            <div style={{fontSize:10,color:"#8888a0",letterSpacing:1}}>GESTIÃN</div>
          </div>
        </div>
      ...
      <div style={{flex:1,padding:"1.25rem",maxWidth:1200,width:"100%",margin:"0 auto"}}>
        ...
        {alertasCriticas.length>0&&tab!=="inventario"&&(
          <div onClick={()=>setTab("inventario")} style={{background:"#451a03",color:"#fcd34d",border:"1px solid #92400e",borderRadius:8,padding:"10px 14px",fontSize:13,marginBottom:12,cursor:"pointer"}}>
            â ï¸ {alertasCriticas.length} referencia(s) con stock crÃ­tico. Ver inventario â
          </div>
        )}
        ...
        {tab==="cotizador"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:"1rem",alignItems:"start"}}>
            <div>
              <Card>
                <CardTitle>â¡ Cotizador</CardTitle>
                <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:12}}>
                  <FG label="Referencia"><div style={{gridColumn:"1/-1"}}>
                    <select value={refId} onChange={e=>{setRefId(e.target.value);setTalla(REFS.find(r=>r.id===e.target.value)?.cat==="NiÃ±o"?TALLAS_NINO[0]:TALLAS_ADULTO[2])}}>
                      {REFS.map(r=><option key={r.id} value={r.id}>{r.name} â {cop(r.cost)}</option>)}
                    </select>
                  </div></FG>
                  ...
                  <FG label="DiseÃ±o / Arte"><input type="text" placeholder="Ej: Logo pecho + espalda" value={diseno} onChange={e=>setDiseno(e.target.value)}/></FG>
                  <FG label="Tipo impresiÃ³n"><select value={tipoImp} onChange={e=>setTipoImp(e.target.value)}>{TIPOS_IMP.map(t=><option key={t}>{t}</option>)}</select></FG>
                  <FG label="CmÂ² estampado"><input type="number" min={0} value={cmEst} onChange={e=>setCmEst(+e.target.value)}/></FG>
                ...
                {(()=>{const st=stockDe(refId,talla,color);return st>=0&&<div style={{fontSize:12,color:st<=3?"#f59e0b":st===0?"#e63d3d":"#22c55e",marginBottom:10}}>ð¦ Stock disponible ({color} Â· {talla}): <strong>{st} unidades</strong></div>;})()}
                <Btn onClick={addToQuote}>â Agregar a cotizaciÃ³n</Btn>
              </Card>

              {/* Items cotizaciÃ³n */}
              {quote.length>0&&(
                <Card>
                  <CardTitle>ð Items en cotizaciÃ³n</CardTitle>
                  {quote.map((item,i)=>(
                    <div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #2a2d3a"}}>
                      <div>
                        ...
                        <div style={{fontSize:12,color:"#8888a0"}}>{item.talla} Â· {item.color} Â· x{item.qty}</div>
                        {item.diseno&&<div style={{fontSize:11,color:"#8888a0"}}>DiseÃ±o: {item.diseno}</div>}
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <Btn sm onClick={()=>setQuote(q=>q.filter((_,j)=>j!==i))} variant="outline">â</Btn>
                      </div>
                    </div>
                  ))}
                  <hr style={{border:"none",borderTop:"1px solid #2a2d3a",margin:"12px 0"}}/>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                    <span style={{color:"#8888a0",fontSize:13}}>TOTAL COTIZACIÃN</span>
                    <span style={{fontSize:24,fontWeight:700,color:"#e63d3d"}}>{cop(quote.reduce((a,i)=>a+i.calc.precioTotal,0))}</span>
                  </div>
                  <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                    <Btn onClick={registrarVenta}>â Registrar Venta</Btn>
                    <Btn variant="outline" onClick={()=>window.print()}>ð¨ï¸ Imprimir</Btn>
            ...
            <Card style={{position:"sticky",top:70}}>
              <CardTitle>ðµ Precio calculado</CardTitle>
              {[
                ["Costo base",cop(ref.cost)],
                cmEst>0&&["Estampado ("+cmEst+"cmÂ²)",cop(cmEst*150)],
          ...
          <>
            ...
            <Card>
              <CardTitle style={{display:"flex",justifyContent:"space-between"}}>
                <span>ð Historial de ventas</span>
                <Btn sm variant="outline" onClick={()=>exportCSV(`furia_ventas_${today()}.csv`,["Fecha","Cliente","Referencia","Talla","Color","Cant.","P.Unit","Total","Ganancia","Sede"],ventas.map(v=>[v.fecha,v.cliente,v.ref,v.talla,v.color,v.cantidad,v.precio,v.totalVenta,v.ganancia,v.sede]))}>â¬ï¸ CSV</Btn>
              </CardTitle>
                    ...
                    <tbody>
                      {[...ventas].reverse().map(v=>(
                        <tr key={v.id} style={{borderTop:"1px solid #2a2d3a"}}>
                          ...
                          <td style={{padding:"10px 12px"}}>{v.cliente||"â"}</td>
                          ...
                          <td style={{padding:"10px 12px"}}>{v.sede||"â"}</td>
                          <td style={{padding:"10px 12px"}}><Btn sm variant="outline" onClick={()=>{if(confirm("Â¿Eliminar?"))saveVentas(ventas.filter(x=>x.id!==v.id))}}>ð</Btn></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {tab==="compras"&&(
          <>
            ...
            <Card>
              <CardTitle>ð¦ Registrar compra / ingreso de stock</CardTitle>
              ...
              <Btn variant="green" onClick={registrarCompra}>â Registrar compra e ingresar stock</Btn>
            </Card>
            <Card>
              <CardTitle style={{display:"flex",justifyContent:"space-between"}}>
                <span>ð Historial de compras</span>
                <Btn sm variant="outline" onClick={()=>exportCSV(`furia_compras_${today()}.csv`,["Fecha","Referencia","Talla","Color","Cant.","P.Unit","Total","Proveedor"],compras.map(c=>[c.fecha,c.ref,c.talla,c.color,c.cantidad,c.precio,c.total,c.proveedor]))}>â¬ï¸ CSV</Btn>
              </CardTitle>
                    ...
                    <tbody>
                      {[...compras].reverse().map(c=>(
                        <tr key={c.id} style={{borderTop:"1px solid #2a2d3a"}}>
                          ...
                          <td style={{padding:"10px 12px"}}>{c.proveedor||"â"}</td>
                          <td style={{padding:"10px 12px"}}><Btn sm variant="outline" onClick={()=>{if(confirm("Â¿Eliminar?"))saveCompras(compras.filter(x=>x.id!==c.id))}}>ð</Btn></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <>
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginBottom:"1rem"}}>
              {[
                ...
                ["Bajo (â¤5)",inventario.filter(i=>i.stock>2&&i.stock<=5).length,"#f59e0b"],
                ["CrÃ­tico (â¤2)",inventario.filter(i=>i.stock>=0&&i.stock<=2).length,"#e63d3d"],
              ].map(([l,v,c]:any)=>(
                <div key={l} style={{background:"#16181f",border:"1px solid #2a2d3a",borderRadius:12,padding:"1rem"}}>
                  <div style={{fontSize:22,fontWeight:700,color:c,marginBottom:2}}>{v}</div>
                  <div style={{fontSize:12,color:"#8888a0"}}>{l}</div>
            ...
            <Card>
              <CardTitle style={{display:"flex",justifyContent:"space-between"}}>
                <span>ð Inventario en tiempo real</span>
                <Btn sm variant="outline" onClick={()=>exportCSV(`furia_inventario_${today()}.csv`,["Referencia","Cat.","Talla","Color","Comprado","Vendido","Stock"],inventario.map(i=>[i.ref,i.cat,i.talla,i.color,i.comprado,i.vendido,i.stock]))}>â¬ï¸ CSV</Btn>
              </CardTitle>
                    ...
                    <tbody>
                      {[...inventario].sort((a,b)=>a.stock-b.stock).map((i,idx)=>(
                        <tr key={idx} style={{borderTop:"1px solid #2a2d3a"}}>
                          ...
                          <td style={{padding:"10px 12px"}}>{i.cat||"â"}</td>
                          ...
                          <td style={{padding:"10px 12px"}}>
                            {i.stock<=2?<Pill color="red">ð´ CrÃ­tico</Pill>:i.stock<=5?<Pill color="yellow">ð¡ Bajo</Pill>:<Pill color="green">ð¢ OK</Pill>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          return (
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
                <Card>
                  <CardTitle>ð Top referencias</CardTitle>
                  {top.length===0?<div style={{textAlign:"center",padding:"1.5rem",color:"#8888a0"}}>Sin ventas aÃºn</div>:
                    top.map((t:any,i:number)=>(
                      <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #2a2d3a"}}>
                        <div style={{fontSize:13}}><span style={{color:"#8888a0",marginRight:8}}>#{i+1}</span>{t.ref.slice(0,30)}{t.ref.length>30?"â¦":""}</div>
                ...
                <Card>
                  <CardTitle>â± Ãltimas ventas</CardTitle>
                  {ultimas.length===0?<div style={{textAlign:"center",padding:"1.5rem",color:"#8888a0"}}>Sin ventas aÃºn</div>:
                    ultimas.map(v=>(
                      <div key={v.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:"1px solid #2a2d3a"}}>
                        <div>
                          ...
                          <div style={{fontSize:11,color:"#8888a0"}}>{v.fecha} Â· {v.ref.slice(0,25)}</div>
                        </div>
              ...
              <Card style={{marginTop:0}}>
                <CardTitle>â¡ Acciones rÃ¡pidas</CardTitle>
                <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
                  <Btn onClick={()=>setTab("cotizador")}>ð° Nueva cotizaciÃ³n</Btn>
                  <Btn variant="outline" onClick={()=>setTab("compras")}>ð¦ Registrar compra</Btn>
                  <Btn variant="outline" onClick={()=>{exportCSV(`furia_ventas_${today()}.csv`,["Fecha","Cliente","Ref","Talla","Color","Cant","Total","Ganancia"],ventas.map(v=>[v.fecha,v.cliente,v.ref,v.talla,v.color,v.cantidad,v.totalVenta,v.ganancia]));setTimeout(()=>exportCSV(`furia_compras_${today()}.csv`,["Fecha","Ref","Talla","Color","Cant","Total","Proveedor"],compras.map(c=>[c.fecha,c.ref,c.talla,c.color,c.cantidad,c.total,c.proveedor])),400)}}>â¬ï¸ Exportar todo</Btn>
                  <Btn sm variant="outline" onClick={()=>{if(confirm("Â¿Borrar TODOS los datos? No se puede deshacer.")){saveVentas([]);saveCompras([]);setQuote([])}}} style={{marginLeft:"auto",color:"#8888a0"}}>ð Resetear</Btn>
                </div>
              </Card>
            </>
          );
